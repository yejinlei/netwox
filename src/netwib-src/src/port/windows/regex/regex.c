...
/* For platform which support the ISO C amendment 1 feature we
...
# define REGEX_FREE(x) free((void*)x)
...
# define REGEX_REALLOCATE(source, osize, nsize)                (destination = (char *) alloca (nsize),                \
...
# define REGEX_ALLOCATE_STACK(size)                 r_alloc (&failure_stack_ptr, (size))
# define REGEX_REALLOCATE_STACK(source, osize, nsize)        r_re_alloc (&failure_stack_ptr, (nsize))
# define REGEX_FREE_STACK(ptr)                    r_alloc_free (&failure_stack_ptr)
...
#ifdef MBS_SUPPORT
# define STORE_NUMBER(destination, number) do {*(destination) = (US_CHAR_TYPE)(number);    } while (0)
#else
     ...
     struct re_pattern_buffer *bufp;
{
  /* We fetch characters from PATTERN here.  Even though PATTERN is
     ...
     they can be reliably used as array indexes.  */
     ...
     struct re_pattern_buffer *bufp;
{
  ...
  /* This holds the pointer to the failure stack, when
     it is allocated to be relocated.  */
...
/* Using the compiled pattern in BUFP->buffer, first tries to match the
   ...
   In REGS, return the indexes of the virtual concatenation of STRING1
              ...
              else
        {
          ...
          assert (bufp->regs_allocated == REGS_FIXED);
              ...
              /* Convert the pointer data in `regstart' and `regend' to
                 indexes.  Register zero has to be set differently,
