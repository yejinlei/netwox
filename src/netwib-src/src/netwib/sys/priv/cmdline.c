...
/*-------------------------------------------------------------*/
static netwib_err netwib_priv_cmdline_init2(netwib_buf *pbufcommand,
                                            netwib_string *pitem)
{
                                    ...
                                    netwib_string *pargv[])
{
  ...
  /* obtain filename */
  netwib_er(netwib_priv_cmdline_init2(&bufcommand, &filename));
  ...
  while (NETWIB_TRUE) {
    ...
    /* store item */
    ret = netwib_priv_cmdline_init2(&bufcommand, &(argv[i]));
    if (ret != NETWIB_ERR_OK) {
...
netwib_err netwib_priv_cmdline_win(netwib_constbuf *pbufcommand,
                                   netwib_buf *poutbuf)
{
  netwib_string filename, *argv, pc;
  int i;
  ...
  /* What's done :
      ...
      - add " around args containing a space (under Windows,
        there is no escape char, so data inside quotes cannot
        be quoted using NETWIB_ENCODETYPE_QUOTED)
  */
  ...
  pc = netwib_c_strchr(filename, ' ');
  if (pc != NULL) {
    netwib_er(netwib_buf_append_byte('"', poutbuf));
  }
  netwib_er(netwib_buf_append_string(filename, poutbuf));
  if (pc != NULL) {
    netwib_er(netwib_buf_append_byte('"', poutbuf));
  }
  ...
  while (argv[i] != NULL) {
    netwib_er(netwib_buf_append_byte(' ', poutbuf));
    pc = netwib_c_strchr(argv[i], ' ');
    if (pc != NULL) {
      netwib_er(netwib_buf_append_byte('"', poutbuf));
    }
    netwib_er(netwib_buf_append_string(argv[i], poutbuf));
    if (pc != NULL) {
      netwib_er(netwib_buf_append_byte('"', poutbuf));
    }
    i++;
