...
/*-------------------------------------------------------------*/
netwib_conststring t000147_description[] = {
  NETWOX_DESC_spoof_packet,
  NETWOX_DESC_spoofip,
  NETWOX_DESC_toolpriv_spoof,
  NULL
};
netwox_toolarg t000147_args[] = {
...
netwib_err t000147_core(int argc, char *argv[])
{
  ...
  /* obtain parameters */
  netwib_er(netwox_arg_init(argc, argv, &t000147_info, &parg));
  ...
  netwib_er(netwib_pkt_append_ipicmp6(&ipheader, &icmp6, &pkt));
  ...
  if (isset) {
    /* decode */
    netwib_er(netwib_pkt_decode_ipicmp6(&pkt, &ipheader, &icmp6));
    /* change fields */
    netwib_er(netwox_arg_isset(parg, 'e', &isset));
    if (isset) {
