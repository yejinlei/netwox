...
/*-------------------------------------------------------------*/
netwib_conststring t000038_description[] = {
  NETWOX_DESC_spoof_packet,
  NETWOX_DESC_spoofip,
  NETWOX_DESC_toolpriv_spoof,
  NULL
};
netwox_toolarg t000038_args[] = {
  NETWOX_TOOLARG_OPTA_SPOOFIP('a', NULL, NULL, NULL),
  NETWOX_TOOLARG_OPTA_UINT32('p', "ip4-ihl", "IP4 ihl", "5"),
  NETWOX_TOOLARG_OPT_UINT32('c', "ip4-tos", "IP4 tos", NULL),
  NETWOX_TOOLARG_OPTA_UINT32('q', "ip4-totlen", "IP4 totlen", NULL),
  ...
  NETWOX_TOOLARG_OPTA_UINT32('r', "ip4-checksum", "IP4 checksum", NULL),
...
netwox_tooltreenodetype t000038_nodes[] = {
  NETWOX_TOOLTREENODETYPE_SPOOF_IP,
  NETWOX_TOOLTREENODETYPE_END
};
netwox_tool_info t000038_info = {
  "Spoof Ip4 packet",
  t000038_description,
  "hping, send",
  t000038_args,
...
netwib_err t000038_core(int argc, char *argv[])
{
  ...
  /* obtain parameters */
  netwib_er(netwox_arg_init(argc, argv, &t000038_info, &parg));
  ...
  netwib_er(netwib_pkt_append_ipdata(&ipheader, &ip4data, &pkt));
  ...
  if (isset) {
    /* decode */
    netwib_er(netwib_pkt_decode_ipdata(&pkt, &ipheader, &ip4data));
    /* change fields */
    netwib_er(netwox_arg_isset(parg, 'p', &isset));
    if (isset) {
      netwib_er(netwox_arg_uint8(parg, 'p', &ipheader.header.ip4.ihl));
    }
    netwib_er(netwox_arg_isset(parg, 'q', &isset));
    if (isset) {
      netwib_er(netwox_arg_uint16(parg, 'q', &ipheader.header.ip4.totlen));
    }
    netwib_er(netwox_arg_isset(parg, 'r', &isset));
    if (isset) {
      netwib_er(netwox_arg_uint16(parg, 'r', &ipheader.header.ip4.check));
    }
