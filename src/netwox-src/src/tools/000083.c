...
/*-------------------------------------------------------------*/
netwib_conststring t000083_description[] = {
...
netwox_tooltreenodetype t000083_nodes[] = {
  NETWOX_TOOLTREENODETYPE_NETAUDIT_ICMP,
  NETWOX_TOOLTREENODETYPE_END
};
netwox_tool_info t000083_info = {
...
netwib_err t000083_core(int argc, char *argv[])
{
  ...
  netwib_er(netwox_arg_init(argc, argv, &t000083_info, &parg));
  ...
  if (info.iptype == NETWIB_IPTYPE_IP4) {
    info.type4 = NETWIB_ICMP4TYPE_TIMEEXCEED;
    netwib_er(netwox_arg_uint32(parg, 'c', (netwib_uint32*)&info.code4));
  } else {
    info.type6 = NETWIB_ICMP6TYPE_TIMEEXCEED;
